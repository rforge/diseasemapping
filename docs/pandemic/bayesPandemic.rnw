...
<<setup,fig=false>>=
options(SweaveHooks=list(fig=function() par(mar=c(2.5,2.5,0.1,0.1), mgp=c(1.5, 0.5, 0), cex=0.6)), width=58)
...
\begin{description}
	...
	\item[Deadly:] Following hospitalization, the patient dies of the infection.
\end{description}
When a case is first identified, it's not known what type it is.
...
\begin{itemize}
	\item Fix the parameter values
	\item Simulate the missing data: infection type, times of infection, onset, hospitalization, recovery, and/or death.
	\item Can simulate directly, as conditioning is easy.
\end{itemize}
\end{block}
...
params = pandemicParams( 
  ...
  MedHospS = c(mean = 2, shape = 2, zeros = 0.1),
  MedHospD = c(mean = 2, shape = 2, zeros = 0.1),
	HospRec = c(mean=2, shape=1.5, zeros=.01 ),
	HospDeath = c(mean=2, shape=0.8, zeros=.05 ),
 	probs = c(M=0.7, S=0.25, D=0.05)
# 	probs = c(M=0.979, S=0.02, D=0.001)
  )

data = simEpidemic(params, delta=4, days=20)
...
paramSample = mcmcPandemic(data,params,priors,sigma=scale,runs=60)
paramSample = paramSample[seq(100,dim(paramSample)[1], by=5),]
dim(paramSample)
...
postSummary[1:12,-c(1,4)]
@
\end{frame}


\begin{frame}
\frametitle{Posterior and prior, Hosp $\Rightarrow$ Death }
...
hospSample = simHospitals(paramSample , Ndays=20, NinfectionsPerDay=20)
...
# generate hospital times
hospSample2 = simHospitals(paramSample , Ndays=40, 
	NinfectionsPerDay=c(seq(from=1,by=2, len=10), seq(from=20, len=10, by=2), rep(2, 20))) 
