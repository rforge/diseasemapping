...
\begin{frame}
\frametitle{Posterior and prior, fatality prob }
<<priorPost, fig=false,results=tex>>=
library(ellipse)
plotPrior(priors, paramSample, transition="HospDeath",file="postFatality", tex=TRUE)
@
\end{frame}
...
hospSample = simHospitals(paramSample , Ndays=20, NinfectionsPerDay=10)
