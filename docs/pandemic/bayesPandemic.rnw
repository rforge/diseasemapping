...
\comment{
<<setup,fig=false>>=
options(SweaveHooks=list(fig=function() par(mar=c(2.5,2.5,0.1,0.1), mgp=c(1.5, 0.5, 0), cex=0.6)), width=54)
...
params = pandemicParams( 
  ...
  MedHospD = c(mean = 2, shape = 2, zeros = 0.1),
	...
	HospDeath = c(mean=5, shape=0.8, zeros=.05 ),
ageProbs=list(
	S=addAgeProbs(ageSeq, 0.02+0.8*dnorm(ageSeq, mean=20, sd=10)),
	D=addAgeProbs(ageSeq, 0.01+0.5*dnorm(ageSeq, mean=30, sd=5))  ),
	probs=NULL
	)
...
<<probS>>=
plot(params$age$S, type="l", ylim=c(0,0.06))
@
\end{block}
...
\begin{block}{Death}
<<probD>>=
plot(params$age$D, type="l", ylim=c(0,0.06))
@
...
data = simEpidemic(params, delta=20, days=28)
