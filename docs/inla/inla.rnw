...
library(glmmBUGS)
bac = glmmBUGS(y ~ trt, effects='ID', family='binomial',data=bacteria, file='modelx.bug')
...
#levels(bacterianew$trt) <- c("placebo", "drug", "drugplus")
#bacteria=bacterianew
# do this pql = glmmPQL(y ~ trt, random = ~1|ID, family='binomial',data=bacteria)
