...
// 2d array
// ie 128 by 4 global, 1 by 4 local
...
__kernel void cholDiag(
	...
	const int Dlocal = get_local_id(0) * get_local_id(1);
	const int Dglobal = get_global_id(0) * get_global_id(1);
	const int Dgroup = get_group_id(0) * get_group_id(1);

	const int Nsize = get_global_size(0) * get_global_size(1);
	const int NlocalSize = get_local_size(0) * get_local_size(1);
...
__kernel void cholOffDiagGroupwise(
	...
	const int Nsize = get_global_size(0)*get_global_size(1);
	const int NlocalSize = get_local_size(0)*get_local_size(1);
	const int Ngroups = get_num_groups(0)*get_num_groups(1);
	...
	for(Drow = Dcol + 1 + Dgroup; Drow < N; Drow += Ngroup) {
...
__kernel void cholCrossprod(
	...
	const int Dlocal = get_local_id(0) * get_local_id(1);
	const int Nlocal = get_local_size(0)*get_local_size(1);
	const int Dglobal = get_global_id(0) * get_global_id(1);
	const int Nglobal = get_global_size(0)*get_global_size(1);
