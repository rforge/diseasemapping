...
myParams = c(shape=2.15, range=1.5, variance = 2, nugget = 0, 
             anisoRatio = 1,#10, 
...
```{r simpleTestFloat}
  ...
  coordsGpu = vclMatrix(-99, nrow(coordsSp@coords), ncol(coordsSp@coords), type="float")

    # this shouldnt be necessary
  # but something currently wrong with gpuR on the server

  coordsGpu[,2] = coordsSp@coords[,2]
  print(coordsGpu[1:4,1:2])

  coordsGpu[,1] = coordsSp@coords[,1]
  print(coordsGpu[1:4,1:2])
...
if(gpuInfo()$double_support) {
  ...
  coordsGpuD = vclMatrix(-99, nrow(coordsSp@coords), ncol(coordsSp@coords), type="double")

    # this shouldnt be necessary
  # but something currently wrong with gpuR on the server

  coordsGpuD[,2] = coordsSp@coords[,2]
  print(coordsGpuD[1:4,1:2])

  coordsGpuD[,1] = coordsSp@coords[,1]
  print(coordsGpuD[1:4,1:2])

  coordsGpuD[,2] = coordsSp@coords[,2]
  print(coordsGpuD[1:4,1:2])
  
  system.time(varGpuD <- geostatsgpu::maternGpu(
    x= coordsGpuD, param=myParams))
} else {
  varGpuD = NA
}
```
...
quantile(varCpu - as.matrix(varGpu), na.rm=TRUE)
quantile(varCpu - as.matrix(varGpuD), na.rm=TRUE)
```
