...
varMatGpu <- vclMatrix(data=-1, length(coordsSp), length(coordsSp), type='float')
```

Create matern matrix on gpu
```{r maternMatGpu}
system.time(varCpu <- geostatsp::matern(coordsSp, myParams, form='variance', type_flag=1))
system.time(geostatsgpu::maternGpu(x=coordsGpu, param=myParams, output=varMatGpu, form='variance',type_flag=1))
...
#cholTest[SSeq,SSeq]
#as.matrix(x3Chol$L)[SSeq,SSeq]

cbind(DfromR[SSeq], as.vector(D)[SSeq], as.vector(x3Chol$D)[SSeq])
...
```{r maternMatGpuChol}
varMatChol = maternGpu(myCoords, param=c(range=1, shape=1.5, variance=2), type='cholesky')
varMatChol$logDet
quantile(as.vector(varMatChol$D - x3Chol$D))
```
