...
system.time(logDet <- geostatsgpu:::cpp_cholGpu(
        x, D, diagWorking, diagTimesRowOfA,
...
SSeq = seq(by=1, len=6, to=ncol(x))
SSeq = 1:5

SSeq = seq(5000, len=5)

myMatR[SSeq, SSeq]
t(as.matrix(x)[SSeq,SSeq])
cholTest[SSeq,SSeq]

cbind(DfromR[SSeq],
as.vector(D)[SSeq])

range(which(abs(DfromR - as.vector(D)) > 0.001))

range(which(abs(cholTest[ncol(x), ] - as.matrix(x)[,ncol(x)]) > 0.01))

plot(cholTest[ncol(x),] - as.matrix(x)[ ,ncol(x)])
...
x = gpuR::deepcopy(myMat)
system.time(x2 <- gpuR:::chol.vclMatrix(x))
