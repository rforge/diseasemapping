...
```{r gpuObjects}
coordsGpu = vclMatrix(coordsSp@coords, type='double')
```

Create matern matrix on gpu
```{r maternMatGpu}
system.time(varCpu <- geostatsp::matern(coordsSp, myParams))
system.time(varGpu <- geostatsgpu::maternGpu(x=coordsGpu, param=myParams, form='cholesky'))
#range(varCpu - as.matrix(varGpu))
```
...
varMat1 = gpuR::deepcopy(varMatGpu)
system.time(varCholGpu <- geostatsgpu:::cholGpu(varMat1))
```

Using `gpuR`'s Cholesky
```{r cholGpurPackage, eval=FALSE}
varMat2 = gpuR::deepcopy(varMatGpu)
system.time(varCholGpuR <- gpuR:::chol.vclMatrix(varMat2))
```

Using ViennaCL's LU decomposition
```{r cholViennaclLu, eval=FALSE}
varMat3 = gpuR::deepcopy(varMatGpu)
system.time(varCholVcl <- luGpu(varMat3))
```

```{r cholGpuCompare, eval=FALSE}
xchold = diag(varCholGpu)
