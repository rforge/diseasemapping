...
system.time(myMatR <- as.matrix(geostatsp::matern(
...
myMatGpuR = gpuR::deepcopy(myMat)

D = gpuR::vclVector(0, nrow(myMat),
            type='double',
            ctx_id = myMat@.context_index)
...
x = gpuR::deepcopy(myMat)
system.time(fromC <- geostatsgpu:::cpp_cholGpu(
        x@address, 
        D@address,
...
SSeq = seq(by=1, len=9, to=ncol(x))
#SSeq = 1:10
cbind(gpu=as.vector(D)[SSeq],
    cpu=DfromR[SSeq], 
    diff=as.vector(D)[SSeq] - DfromR[SSeq])
#myMatR[SSeq,SSeq]
round(as.matrix(x)[SSeq,SSeq],3)
round(cholTest[SSeq,SSeq],3)

plot(pmax(1e-17,
    abs(as.vector(D) - DfromR)),
    type='l', log='y',
    ylim = c(1e-18,0.1))
abline(v=control$localStorage*(1:10))

plot(pmax(1e-17,
    abs(as.vector(D) - DfromR))[
    seq(to = length(D), by=1, len=50)
        ],
    type='o', log='y',
    ylim = c(1e-18,0.1))
abline(v=control$localStorage*(1:10))
...
cholFromGpu = t(as.matrix(x))
cholFromGpu[!lower.tri(cholFromGpu, diag=TRUE)] = NA
quantile(cholFromGpu - cholTest, na.rm=TRUE)

plotFrac = 0.01
raster::plot(log(abs(raster::crop(
    raster::raster(
        cholFromGpu - cholTest,
        1, ncol(x), 1, ncol(x)
        ),
    raster::extent(
        ncol(x) * c(1-plotFrac, 1, 0, plotFrac))
    ))))
abline(v=(1:10)*control$localStorage)

system.time(cholGpuR <- 
    gpuR:::chol.vclMatrix(myMatGpuR))

quantile(DfromR - as.vector(D))


system.time(maternChol <- 
    geostatsgpu:::cholGpu(
        myMat, 
        verbose=TRUE, 
        control = control))

quantile(DfromR - as.vector(maternChol$D))
...
system.time(maternChol2 <- geostatsgpu:::cholGpu(myMat2, 
    MClocal = 128,
    verbose=TRUE))
quantile(DfromR - as.vector(maternChol2$D))
...
system.time(maternChol3 <- geostatsgpu:::cholGpu(myMat3, 
    MClocal = 256,
    verbose=TRUE))
quantile(DfromR - as.vector(maternChol3$D))
...
system.time(maternChol4 <- geostatsgpu:::cholGpu(myMat4, 
    MClocal = 512,
    verbose=TRUE))
quantile(DfromR - as.vector(maternChol4$D))
}
```

```{r cholRes, eval=FALSE}

LfromG = t(as.matrix(maternChol2$L))
DfromG = as.vector(maternChol2$D)
...
Sind = seq(to=ncol(cholTest), by=1, len=5)
LfromG[Sind,Sind]
cholTest[Sind,Sind]
```
...
min(which(abs(DfromG - DfromR) > 0.1))
plot(DfromG - DfromR)
```
