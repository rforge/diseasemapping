...
```{r paramsBatch}
myParamsBatch = rbind(myParams, myParams, myParams * 0.8, myParams*1.2)
...
coordsGpu2 = vclMatrix(cbind(1:3, 1:3), 
    type=gpuR::typeof(paramsGpu))
outputBatchF = vclMatrix(0, 
    nrow(paramsGpu), nrow(coordsGpu2)^2,
    type=gpuR::typeof(paramsGpu))
...
```{r batchMatern}
  geostatsgpu:::maternBatchBackend(
    ...
    numWorkItems = c(4L,4L), 
    numLocalItems = c(2L, 2L))
  t(as.matrix(outputBatchF))
  t(as.matrix(paramsGpu))
```
