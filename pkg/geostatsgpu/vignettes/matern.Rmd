...
```{r paramsBatch}
myParams = c(shape=2.15, range=0.25, variance = 2, nugget = 0, 
             anisoRatio = 4, 
...
badone = min(which(abs(toCompare[,1]) > 0.01))
badone
if(length(badone)) {
  toCompare[badone + seq(-1, 4), ]
  as.matrix(simMat)[badone + seq(-1, 4), ]
  multCpu[badone + seq(-1, 4), ]
}

toCompare[2*ncol(toCompare) + seq(-1, 4), ]
...
plot(simRaster, nc = Nsim, nr=Nmatrix)
