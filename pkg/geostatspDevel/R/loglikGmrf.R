...
#  dyn.load('../src/gmrfLik.so')
  ...
  if(length(boxcox)>1){
    ...
    ml[,,'boxcox',] = boxcox
      ...
      newml = array(NA, c(1,dim(ml)[-1]),
          dimnames=c(list(NULL),dimnames(ml)[-1]))
      newssq =array(NA, 
          c(1+ncol(Xmat),
 ...
 # retain only propNugget with best likelihood
    ...
    forml = abind::abind(
        ...
        oneminusar = array(oneminusar, dim(ml)[-3]),
        along=3
    )
    dimnames(forml)[[3]] = c(
    ...
    parArray = array(parMat,c(dim(ml)[1:2],dim(parMat)))
  ...
  ml = abind::abind(ml,
      boxcox=array(boxcox, dim(ml)[-3]),
    along=3)

  ml = aperm(ml, c(4,1,2,3))
  ml = abind::abind(ml,
      oneminusar=array(oneminusarRes, dim(ml)[-4]),
      along=4)

  mlMat = matrix(ml, ncol=dim(ml)[[4]], 
      dimnames=list(NULL, dimnames(ml)[[4]]))
  toKeep = !is.na(mlMat[,'det'])
  mlMat = mlMat[toKeep,]
  mlMat = cbind(mlMat, propNugget = 1/mlMat[,'xisqTausq'])
  
  mle= which.min(mlMat[,Lcol])
  minL = mlMat[mle,Lcol]
  mlMat = cbind(mlMat, deviance = mlMat[,Lcol]-minL)
  whichGood = which(mlMat[,'deviance']<8)
  if(length(whichGood)>2)
    mlMat = mlMat[whichGood,]
  
  mle= mlMat[which.min(mlMat[,Lcol]),]
  
  mleMl = ml[as.character(mle['oneminusar']),as.character(mle['boxcox']),,]
  toKeep = c(as.character(mle['boxcox']), colnames(Xmat))
  
  mleSsq = ssq[toKeep[-1],toKeep,
      which.min(mleMl[,Lcol]),
      as.character(mle['oneminusar'])]
