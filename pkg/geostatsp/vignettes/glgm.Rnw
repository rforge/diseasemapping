...
if(all(havePackages)) {
  ...
  swissFitNp = glgm(
    ...
    prior=list(sd=c(u = 0.5, alpha = 0.1), range=c(50000,500000), sdObs = c(u=1, alpha=0.4)), 
...
if(all(havePackages)) {
  swissFit =  glgm("lograin", swissRain, Ncell, 
    ...
    control.mode=list(theta=c(2.5, 0.6, 1.6),restart=TRUE)
  )
...
if(all(havePackages) & fact > 1) {
  ...
  loaFit = glgm(
    ...
    prior = list(sd=log(2), range=200*1000)
    )
...
<<LongTestsSwiss, fig.height=7, fig.width=3>>=  
if(all(havePackages) &  fact > 1 ) {
...
<<noData, fig.height=7, fig.width=3>>=  
...
if(all(havePackages) & fact > 1) {
  ...
  res = glgm(data=data2, grid=Ncell, 
    ...
    control.fixed=list(
      ...
      mean=0,prec=4),
    control.mode=list(theta=c(0.6, 1),restart=TRUE)
  )
...
<<noDataQuantile, fig.height=7, fig.width=3>>=  

if(all(havePackages) & fact > 1) {

bob=priorInla(list(
      sd=0.5,#c(lower=0.5, upper=0.7), 
      range=c(lower=0.1, upper=0.11)), cellSize = 0.04
)
  ...
  res = glgm(data=data2, grid=Ncell, 
    ...
    prior = list(
      sd=0.5,#c(lower=0.5, upper=0.7), 
      range=c(lower=0.1, upper=0.11)),
    family="poisson",buffer=1, 
    control.fixed=list(mean.intercept=0, prec.intercept=1,
      mean=0,prec=4), verbose=TRUE#,
#    control.mode=list(theta=c(0, 20),restart=TRUE)
  )
...
<<noDataLegacy, fig.height=7, fig.width=3>>=  
