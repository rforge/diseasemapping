...
<<lgcpPc, tidy=TRUE>>=
resP=lgcp(formula, data=murder, 
    grid=squareRaster(torontoBorder, 30),
    covariates=covList,
...
colFit = colourScale(resG$raster[['fitted.exp']],
		breaks=6, dec=1, style='equal')


map.new(resG$raster)
plot(resG$raster[['fitted.exp']], 
