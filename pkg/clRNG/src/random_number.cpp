    ...
    std::string precision_type) 
{
  if(precision_type == "float") {
    return random_numberGpu<float>(xR, streamsR, max_global_size, max_local_size,random_type);
  } else if (precision_type=="double") {
    return random_numberGpu<double>(xR, streamsR, max_global_size, max_local_size,random_type);
  } else {
    return random_numberGpu<int>(xR, streamsR, max_global_size, max_local_size,random_type);
  }
}
