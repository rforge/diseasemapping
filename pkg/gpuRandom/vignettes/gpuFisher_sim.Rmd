...
```{r}
set.seed(100)
eg<-matrix(as.integer(runif(600,0,1000)), nrow=100, ncol=6)


#setContext(id=3)
#currentDevice()

#system.time(fisher.test(eg, simulate.p.value = TRUE, B = 1e5))

## Using the gpu function
setContext(   grep('gpu', listContexts()$device_type) [2]    )
currentDevice()
 a = as.integer(rowSums(eg, na.rm = FALSE))
 b = as.integer(colSums(eg, na.rm = FALSE))
 a<-as.vclVector(a)
 b<-as.vclVector(b)

#system.time(gpuRandom::gpuFisher_sim(a, b, results, streams, workgroupSize = Nglobal))
