...
a12=matrix(2*seq(1, prod(dima)), dima[1], dima[2])
b11=matrix(seq(1, prod(dimb)), dimb[1], dimb[2])
b12=matrix(2*seq(1, prod(dimb)), dimb[1], dimb[2])
...
A12= cbind(zerocol,rbind(zerorow,a12,zerorow))
zerorow=integer(dimb[2])
zerocol=integer(dimb[1]+2)
B11= cbind(rbind(zerorow,zerorow,b11),zerocol)
B12= cbind(rbind(zerorow,zerorow,b12),zerocol)


gpuA = vclMatrix(cbind(A11,A12), type='float')
gpuB = vclMatrix(cbind(B11,B12), type='float')

outputCp = vclMatrix(matrix(-123, 10, 16), type='float')   # 3x4  3x4
...
gemmBatch2backend(
  ...
  c(2,2,1,0,1,0), # batches: nRow, nCol, recycleArow, recycleAcol, recycleB row col
  c(2,4,4,1,2,2), #workgroupSize  global 0 1 2, local 0 1 2  
  c(16,16), # cacheSizeA, cacheSizeB,                
  TRUE # verbose
  )
