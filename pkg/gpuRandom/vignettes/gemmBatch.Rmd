...
```{r test one vs multiple, echo=TRUE, eval=T}
...
C4<-gpuRandom::gemmBatch(gpuA, gpuB1, 4L, 2L, 1L, FALSE, Nglobal)
as.matrix(C4)

C5<-gpuRandom::gemmBatch(gpuA1, gpuB, 4L, 1L, 2L, FALSE, Nglobal)
as.matrix(C5)

C6<-gpuRandom::gemmBatch(gpuA1t, gpuB, 4L, 1L, 2L, TRUE, Nglobal)
as.matrix(C6)
