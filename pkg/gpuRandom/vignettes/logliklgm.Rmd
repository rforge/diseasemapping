...
```{r test_on_cpu, message=FALSE, eval=FALSE}
library(gpuR)
library(geostatsp)
set.seed(1)
n=20
...
# fit the model second time with different parameters
myres = likfitLgm(
	formula=Y ~ cov1 + cov2, 
	data=mydat, 
	param=c(variance=5.5,range=10, shape=3, nugget=0.75), reml=FALSE)
#variance parameter and regression coefficients are always estimated even if not listed.
# produces mles of parameters
myres$parameters
myres$summary[c("(Intercept)","cov1","cov2"),1] 
...
temp2=gpuRandom::likfitGpu(myres,mydat, type="double", 
                           paramsBatch,betas=NULL,form="ml",workgroupSize,localSize, NlocalCache)
as.matrix(temp2)
#117.924
temp3=gpuRandom::likfitGpu(myres,mydat, type="double", 
                           paramsBatch,betas,form="mlFixSigma",workgroupSize,localSize, NlocalCache)
as.matrix(temp3)
#50.17793

temp4=gpuRandom::likfitGpu(myres,mydat, type="double", 
                           paramsBatch,betas=NULL,form="mlFixBeta",workgroupSize,localSize, NlocalCache)
as.matrix(temp4)
#60.14219

temp5=gpuRandom::likfitGpu(myres,mydat, type="double", 
                           paramsBatch,betas=NULL,form="reml",workgroupSize,localSize, NlocalCache)
as.matrix(temp5)
#67.53707

temp6=gpuRandom::likfitGpu(myres,mydat, type="double", 
                           paramsBatch,betas=NULL,form="remlPro",workgroupSize,localSize, NlocalCache)
as.matrix(temp6)
#66.66792
```
...
```{r test_using_new_example_Nov20,eval=T}
library('geostatsp')
data('swissRain')
sr2 = swissRain[1:20,]
...
swissFitIso = likfitLgm(
  data=sr2,
  formula=sqrtY ~ elev,
  param=c(range=10,shape=1,nugget=0.5),
  reml=FALSE,
  verbose=FALSE
)
...
newParamList = list(
  range=seq(15, 50 , len=1),
  nugget = seq(0.4,0.6,len=1)
)
...
temp=gpuRandom::likfitGpu(swissFitIso,sr2, type="double",
  paramsBatch,betas=NULL,form="mlFixBeta",
