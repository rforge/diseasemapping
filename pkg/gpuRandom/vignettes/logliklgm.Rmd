...
```{r test_using_another_example_Nov20,eval=F}
library('geostatsp')
data('swissRain')
sr2 = swissRain[0:100,]
...
swissFitIso = likfitLgm(
  data=sr2,
  formula=sqrtY ~ elev,
  param=c(range=50000,shape=1,nugget=1.5),
  reml=FALSE,
  verbose=FALSE
)
...
workgroupSize=c(128,16,8)
localSize = c(4,4,1)
NlocalCache=500
 
temp=gpuRandom::likfitGpu(swissFitIso,sr2, type="double",paramsBatch,betas=NULL,form="ml",workgroupSize,localSize,NlocalCache)$minusTwoLogLik
...
newParamList = list(
  range=seq(30000, 80000 , len=290),
  nugget = seq(0,1,len=50)
) #11600


newParam= do.call(expand.grid, newParamList)

otherparams = c(shape=1, variance = 2.021188, anisoRatio = 1, anisoAngleRadians = 0)
otherparams = matrix(rep(otherparams,each=nrow(newParam)),nrow=nrow(newParam))
colnames(otherparams) <- c("shape","variance", "anisoRatio", "anisoAngleRadians")

paramsBatch0 = cbind(newParam, otherparams)
bigparamsBatch0 = t(apply(paramsBatch0, 1, geostatsp::fillParam))
...
system.time(temp <- gpuRandom::likfitGpu(swissFitIso, sr2, type="double", 
                      bigparamsBatch0, #cpu Matrix of parameter sets,
                      betas=NULL, #a vclmatrix  #given by the user or provided from formula
                      form = "ml",  
                      groupsize=290,
                      workgroupSize,
