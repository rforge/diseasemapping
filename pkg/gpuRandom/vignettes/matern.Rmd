...
zMatGpu3 = vclMatrix(nrow=as.integer(ncol(outputBatchF) * Nmatrix),
                     ncol= as.integer(Nsim), 
                     type = gpuR::typeof(outputChol))
zMatGpu2 = vclVector(length=as.integer(ncol(outputBatchF) * Nmatrix * Nsim), 
                     type = gpuR::typeof(outputChol))



streams2 <- gpuRandom::gpuRn(
  zMatGpu2, 
  streams=streams, random_type = "normal")

zMatGpu4 = vclMatrix(as.vector(zMatGpu2), 
                     nrow=as.integer(ncol(outputBatchF) * Nmatrix),
                     ncol= as.integer(Nsim))
```
