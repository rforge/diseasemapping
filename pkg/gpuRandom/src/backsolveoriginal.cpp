...
template <typename T> 
std::string backsolveBatchString2(
    const int sameB,
    const int diagIsOne,
    const int Nrow, 
    const int Ncol,
  ...
  result += "__kernel void backsolveBatch2(\n"
...
void backsolveBatch2(
  ...
  std::string clString =  backsolveBatchString2<T>(  
    Nrow == B.size1(),
    diagIsOne,
    Nrow, 
    Ncol, // ncol
  ...
  viennacl::ocl::kernel & backsolveKernel = my_prog.get_kernel("backsolveBatch2");
...
SEXP backsolveBatchBackend2(
  ...
  if(precision_type == "fvclMatrix") {
    result = backsolveBatchTyped2<float>(
      C, A, B, diagIsOne, Nglobal, Nlocal, NlocalCache);
  } else if (precision_type == "dvclMatrix") {
    result = backsolveBatchTyped2<double>(
      C, A, B, diagIsOne, Nglobal, Nlocal, NlocalCache);
  } else {
    result = Rcpp::wrap(1L);
  }
