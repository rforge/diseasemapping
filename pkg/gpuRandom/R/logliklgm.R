...
likfitGpu <- function(modelname, mydat, type=c("double", "float"), 
  ...
  jacobian<- matrix(jacobian, nrow=groupsize, ncol=length(jacobian), byrow=TRUE)  # make it from a vector to a matrix!!!
  ...
  likfitGpu_Backend(output1$coordsGpu,
                    ...
                    yX, 
                    betas,
                    bigvariances,  #already in the correct form
                    jacobian, # already in matrix form
                    finalLogLik,   
                    output1$n, 
                    output1$p, 
                    groupsize,
