...
if("rerun" %in% theArgs) {
		assign("rerun", TRUE, pos=1)
} else {
		assign("rerun", FALSE, pos=1)
}

library('rgdal')
@

If the following is {\tt TRUE}, the analysis was rerun to produce this vignette.
Otherwise a previously analysis was used.
...
<<rotateToronto,cache=TRUE,include=FALSE>>=
...
fit = lgcp(myLgcp$events, cell=100,covariates=torontoNightR,
		formula = ~layer, shape=2, buffer=1000,
		priorCI = list(sd = c(0.1, 4), range = c(2000, 50000)))
@
<<saveSystemFile,eval=rerun,echo=FALSE>>=
# if a new version of fit.RData has been built
save(myLgcp, fit, file="../inst/extdata/fit.RData")
@

<<loadsystemresults,echo=FALSE,eval=!rerun>>=
...
xtable(fit$par$summary[,c("mean","0.025quant","0.975quant")],
		...
		dec=3)
@
<<inlaResPlot,fig.cap="Intensity surfaces",fig.subcap=c("intensity true","intensity posterior mean", "rand eff true","rand eff posterior mean"),out.width="0.45\\textwidth",echo=FALSE>>=
