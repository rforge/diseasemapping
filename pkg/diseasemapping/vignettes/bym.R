...
#' BYM with gamma priors on precision
...
#+ priorPost, fig.cap='gamma priors, sd parameters', fig.height=4, fig.width=3, fig.subcap=c('spatial','indep'), echo=FALSE, out.width='0.4\\textwidth'
...
#' BYM with penalised complexity prior
#' $pr(sd > 1) = 0.05$ and $pr(phi < 0.2) = 0.95$

#+ bymPc, tidy=TRUE
if(any(names(inla.models()$latent)=='bym2')) {
kBYMpc = suppressWarnings(
bym(
		formula = observed ~ offset(logExpected) + poverty,
    kentucky,
...
#+ priorPostPc, fig.cap='PC priors, variance parameters', fig.height=4, fig.width=3, fig.subcap=c('sd','prop spatial'), out.width='0.4\\textwidth', echo=FALSE
if(!is.null(kBYMpc)) {
...
#+ maps, fig.cap='Random effects and fitted values', fig.subcap=c('gamma, fitted','pc fitted','gamma random','pc random'), fig.height=2, fig.width=5, echo=FALSE

if(require('mapmisc') & !is.null(kBYMpc)) {
