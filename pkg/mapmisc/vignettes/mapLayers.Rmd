...
if(allTiles) {
  ...
  cachePath = file.path(tempdir(), 'mapLayersVignetteCache')
} else {
...
```{r additionalMichelin, echo=TRUE, purl=FALSE, eval=unlist(mget("allTiles", ifnotfound=FALSE))}
...
```{r stamenRetina, echo=TRUE, purl=FALSE, eval=unlist(mget("allTiles", ifnotfound=FALSE))}
...
```{r aditionalEsri, echo=TRUE, purl=FALSE, eval=unlist(mget("allTiles", ifnotfound=FALSE))}
