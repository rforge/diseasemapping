...
```{r osm}
if(haveRgdal){
  map = openmap(x, path='osm-no-labels', maxTiles=12, buffer=c(30,3))
...
```{r mapquest}
if(haveRgdal){
  mapSat = openmap(x, path='opentopomap', maxTiles=4, buffer=c(30,3))
...
if(haveRgdal) {
  ...
  mapLL = openmap(waterhouseLL, buffer=0.02)
...
```{r rockLL}	
if(haveRgdal) {
  mapLLfine = openmap(rockLL, zoom=19, buffer=0.0002)  
  map.new(mapLLfine)
