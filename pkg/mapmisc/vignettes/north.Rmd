...
# fact = 1 
# or run R --slave -e "knitr::knit('north.Rmd')" fact=1
theArgs = grep("^fact=", commandArgs(), value=TRUE)
for(D in theArgs)
  eval(parse(text=D))
...
```{r osm}
if(haveRgdal){
  map = openmap(x, path='osm-no-labels', maxTiles=12, buffer=c(30,3), cachePath=cachePath)
...
```{r mapquest}
if(haveRgdal){
  mapSat = openmap(x, path='opentopomap', maxTiles=4, buffer=c(30,3), cachePath=cachePath)
...
if(haveRgdal){
  mapSat = openmap(x=x[x$name=='Hans island',], 
    path='opentopomap', 
    buffer=c(4,1), zoom=2, cachePath=cachePath)
...
```{r mapquestQaanaaq}
if(haveRgdal){
  mapSat = openmap(x[x$name=='Qaanaaq',], path='maptoolkit', 
    buffer=c(2,1), zoom=5, cachePath=cachePath)
...
if(haveRgdal){
  ...
  mapMerc = openmap(xMerc, path='osm-no-labels',
    zoom=3,
    maxTiles=12, buffer=c(50,200)*1000, cachePath=cachePath)
...
```{r nrcan}
if(haveRgdal){
  xOmerc = spTransform(x, 
    omerc(x,angle=5))
  
  map = openmap(xOmerc, 
    path=c('osm-no-labels', 'stamen-terrain-labels'), 
    fact=1.5, maxTiles=12, 
    verbose=TRUE,
    buffer=c(2,10,10,0)*100*1000, cachePath=cachePath)
  
  mapText = rgbtToIndex(map, pattern='stamen.terrain.labels')
  
  map.new(map)
  plot(map[['osm.no.labels']], add=TRUE)
...
if(haveRgdal){
  map = openmap(x[x$name=='Alex Fjord',], 		
    path='maptoolkit', zoom=3, 
    buffer=c(5,5,2,1), cachePath=cachePath)   
  map.new(map)
  plot(map, add=TRUE)
  points(x, pch=4, col='red', cex=5)
...
if(haveRgdal){
  ...
  southInset = openmap(x=waterhouseWrap, 
    path='osm-no-labels',
    buffer=c(4,1,2,1)*1000*1000, cachePath=cachePath)
  
} else {
  southInset = openmap(waterhouseMerc, 
    path='osm-no-labels',
    buffer=c(4,0,0,1.5)*1000*1000, 
    crs=NA, cachePath=cachePath)
}
```
...
```{r waterhouseMerc} 
mapMerc = openmap(waterhouseMerc, buffer=4000, crs=NA, cachePath=cachePath)
...
mapMercFine = openmap(rockMerc, buffer=100, crs=NA, cachePath=cachePath)
...
if(haveRgdal) {
  ...
  mapLL = openmap(waterhouseLL, buffer=0.02, cachePath=cachePath)
...
```{r rockLL}	
if(haveRgdal) {
  mapLLfine = openmap(rockLL, zoom=19, buffer=0.0002, cachePath=cachePath)  
  map.new(mapLLfine)
