...
opts_chunk$set(echo=TRUE,message=FALSE,
		...
		fig.height=4,fig.width=3,out.width="0.3\\textwidth",
...
# plot elevation
map.new(meuse, rasterLegend=TRUE)
plot(nldTiles, add=TRUE)
plot(nldElev,add=TRUE,col=terrain.colors(8),alpha=0.6)
...
<<reproj,cache=TRUE>>=
rotatedCRS = CRS("+proj=omerc +lat_0=51 +lonc=5.75 
				+alpha=-50.0000000 +gamma=0.0 +k=1.000000 +x_0=500000.000 
...
plot(nldElev,add=TRUE,col=terrain.colors(8),alpha=0.6)
scaleBar(meuse,pos="topleft",bg="white")
insetMap(meuse, "bottomleft",zoom=0)
...
points(meuseRot,col="red", cex=0.3)
scaleBar(proj4string(meuseRot), bg="white")
insetMap(meuseRot, "bottomleft",zoom=6)
@
...
<<elevScale>>=
elevScale = colourScale(values(nldElev), breaks=6, col=terrain.colors,
...
map.new(meuse)
plot(nldTiles, add=TRUE)
plot(meuse, col=cuScale$col,add=TRUE,pch=16)
scaleBar(meuse, pos="topleft",bg="white")
legendBreaks("bottomright",  breaks=cuScale,
		title="gals/firkin",bg="white")


map.new(meuse)
plot(nldTiles, add=TRUE)
plot(meuse, col=soilScale$col,add=TRUE,pch=16)
scaleBar(meuse, pos="topleft",bg="white")
legend("bottomright", fill=soilScale$legendCol, legend=soilScale$breaks,
		title="soil type", cex=0.7,bg="white")

map.new(meuse)
plot(nldTiles, add=TRUE)
image(nldElev, breaks=elevScale$breaks, col=elevScale$withTrans, legend=FALSE,add=TRUE)
scaleBar(meuse, pos="topleft",bg="white")
