...
<<so2Plot,fig.cap='so2 data',out.width='0.5\\textwidth',fig.subcap=c('so2','population'),fig.height=4,fig.width=5,echo=FALSE>>=
plot(china2[['SO2.median']])
plot(world,add=TRUE)
plot(china2[['pop']])
plot(world,add=TRUE)
...
<<chinaForGmrf,cache=TRUE,cache.extra=dim(china2)>>=
chinaForGmrf = 
		list(Xmat = 
			model.matrix(
...
arseq = seq(0.5,1,len=200)[-length(arseq)]
library(parallel)
...
so2oneNugget = logLikGmrfNugget(ar=0.95,
		propNugget = seq(0, len=6,by=0.001),
...
arseqN = seq(0.5,1,len=4)
arseqN=arseqN[-length(arseqN)]
chinaForGmrf$propNugget = seq(0,len=3,by=0.001)
library(parallel)
