...
\hat\beta' \mathbf{X}' V^{-1} \mathbf{X}  \hat\beta = & \mathbf{Y}'  V^{-1}   \mathbf{X}[ \mathbf{X}'
 ...
 \mathbf{Y}' V^{-1} \mathbf{X}  \hat\beta = &\hat\beta' \mathbf{X}' V^{-1} \mathbf{X}  \hat\beta \\
\end{align*}
...
L = & \log| V| +	(1/\tau^2) \left[\mathbf{Y}' \mathbf{Y} + \xi^2   L_Y' L_Y  +L_c L_c^T\right] \\
 \log| V| = &- \log| \tau^2 Q (L L ')^{-1} |\\
 = & -N \log(\tau^2) - \log |Q| + 2 \log |L| \\
 \widehat{\log(\tau^2)} =& - \log N	 +\log\left[\mathbf{Y}' \mathbf{Y} + \xi^2   L_Y' L_Y  +L_c L_c^T\right] \\
L = &N \log\left[\mathbf{Y}' \mathbf{Y} + \xi^2   L_Y' L_Y  +L_c L_c^T\right]  - \log |Q| + 2 \log |L|
 \end{align*}
