...
= &     \tau^2 L^{\prime -1}(  \xi^2/\tau^2 I +    L'L  ) L^{-1}\\
(  \xi^2/\tau^2 I +    L'L  ) = & L_2L_2'\\
\var(\mathbf{Y})^{-1} = &  \tau^2  L L_2^{-1\prime} L_2^{-1}   L' \\
\end{align*}
...
\begin{align*}
-2 \log pr(\mathbf{Y};\phi,\beta,\xi,\tau) = &   - \log|  \tau^2  L(
L_2^{-1\prime} L_2^{-1} ) L'|+ \tau^2  ( \mathbf{Y} -\mathbf{X}\beta)  '   
  L( L_2^{-1\prime} L_2^{-1} ) L'
(\mathbf{Y} - \mathbf{X}\beta)  \\
-2 \log pr(\mathbf{Y};\phi,\xi/\tau,\tau)  = & N \log(\tau^2) - 2 \log(|L|)	 + 2
\log(|L_2|) + \tau^2  \breve Y' \breve Y
\end{align*}
